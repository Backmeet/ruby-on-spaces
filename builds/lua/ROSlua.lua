...
local function lu_len(args_wrapped)
	...
	if is_dict(x) then
		local c = 0
		for k,_ in pairs(x) do if k ~= "__dict" then c = c + 1 end end
...
local function isdir(path)
    ...
    if not ok then
        if code == 13 then return true end -- permission denied → dir
        return false
    end
...
local function listdir(path)
    ...
    local p
    if sep == "\\" then
        p = io.popen('dir "'..path..'" /b')
    else
        p = io.popen('ls -A "'..path..'"')
    end
...
local function attributes(path)
    return { mode = isdir(path) and "directory" or "file" }
end

local function readLibs(path, parent_key)
    parent_key = parent_key or ""
    local result = {}
    for _, entry in ipairs(listdir(path)) do
        if entry ~= "." and entry ~= ".." then
            local full_path = path .. sep .. entry
            local attr = attributes(full_path)
            if attr.mode == "directory" then
                local sub_key = parent_key ~= "" and (parent_key .. "." .. entry) or entry
                local sub_table = readLibs(full_path, sub_key)
                ...
                ext = (ext or ""):lower()
                if ext == "lua" or ext == "txt" or ext == "bin" or ext == "" then
                    local key = parent_key ~= "" and (parent_key .. "." .. name) or name
...
env_basic = ROSL.BasicEnv({})

if #arg == 1 then
    local mode = attributes(arg[1]).mode
    if mode ~= "file" then print("Error: file to run is not a file but a :" .. mode); os.exit() end

    local file = io.open(arg[1], "r")

    ROSL.run(file:read("*a"), env_basic)
    file:close()
elseif #arg > 1 then 
    if arg[2] ~= "--libs" then 
        print("Error: extra arg detected but flag '--libs' not found")
        os.exit()
    end
    ...
    local env = readLibs(arg[3])

    ROSL.run(file:read("*a"), env)

    file:close()
else
    print("Error: cannot properly read or understand cmd line args")
    print("Cmd line arg read: " .. table.unpack(arg))
    print("Usage: lua ROS.luac [path to file] --libs [path to dir of imports]")
    print("OR")
    print("Usage: lua ROS.luac [path to file] --libs [path to a single file to be importable]")
    print("OR")
    print("Usage: lua ROS.luac [path to file]")
end    