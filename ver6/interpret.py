...
try:
    ...
    class array:
        ...
        def __repr__(self):
            return f"array({self.value}, {self.size})"
    ...
    def convetItrable(itrable:str, varContext=None):
        if not varContext:
            varContext = variables
        
        if (itrable.startswith("[") and itrable.endswith("]")):
            return [parseValue(i) for i in itrable[1:-1].split(",")], "list"
        elif (itrable.startswith("(") and itrable.endswith(")")):
            x = [parseValue(i) for i in itrable[1:-1].split(",")]
            return array(x, len(x) - 1), "array"
        elif (itrable.startswith("{") and itrable.endswith("}")):
            items_str = itrable[1:-1].strip()
            if not items_str:
                return {}, "dict"
            result = {}
            for item in items_str.split(","):
                key_part, value_part = item.split(":")
                key, _ = parseValue(key_part.strip(), varContext)
                val, _ = parseValue(value_part.strip(), varContext)
                result[key] = val
            return result, "dict"
        else:
            raise ValueError(f"Value | {itrable} | is not a valid parsed itrable value | line {current_index} in {current_source} |")



    def parseValue(valueStr:str, varContext=None):
        global current_index, variables, current_source
        if not varContext:
            varContext = variables
        if valueStr in varContext:  # Variable lookup
            ...
            if isinstance(val, int) or isinstance(val, float):
                ...
                value_type = "itrable list"
            elif isinstance(val, array):
                value_type = "itrable array"
            elif isinstance(val, dict):
                value_type = "itrable dict"
            ...
            return convetItrable(valueStr), "list"
        elif (valueStr.startswith("(") and valueStr.endswith(")")):
            return convetItrable(valueStr), "array",
        elif (valueStr.startswith("{") and valueStr.endswith("}")):
            return convetItrable(valueStr), "dict"
        else:
            raise ValueError(f"Value | {valueStr} | is not valid value | line {current_index} in {current_source} |")
            ...
            case "itrables":
                match args[0]:
                    case "new":
                        ...
                        if parsed1[1] not in ["array", "dict", "list"]:
                            raise ValueError("Can not set a itrable to a constant value")
                    ...
                    case "do":
                        ...
                        if len(args) == 6 and args[4] == "=":
                            ...
                            if parsed1[1] in ["itralbe array", "itralbe list"] and parsed2[1] in ["var int", "literal int", "itralbe array", "itralbe list", "itralbe dict", "var str", "literal str"]:
                                value1 = parsed1[0]
                                value2 = parsed2[0]
                                match op:
                                    case "append": result = value1.append(value2)
                                    case "pop": result = value1.pop(value2)
